...
public class ClientProxy extends CommonProxy {
    ...
    public void init(FMLJavaModLoadingContext context) {
        context.getModEventBus().addListener(ClientProxy::onAddLayers);
        context.getModEventBus().addListener(ClientProxy::setupParticles);
    }
...
//        if (event.isAttack() && PetHomeMod.CONFIG.swingThroughPets.get() && !Minecraft.getInstance().player.isShiftKeyDown() && Minecraft.getInstance().hitResult instanceof EntityHitResult && TameableUtils.isPetOf(Minecraft.getInstance().player, ((EntityHitResult) Minecraft.getInstance().hitResult).getEntity())) {
    ...
    private void renderNametagEnchantments(Entity entity, Component nameTag, PoseStack pose, MultiBufferSource buffer, int lightIn) {
        if (Minecraft.getInstance().player.isShiftKeyDown() && TameableUtils.isTamed(entity) && TameableUtils.hasAnyEnchants((LivingEntity) entity)) {
            ...
            if (net.minecraftforge.client.ForgeHooksClient.isNameplateInRenderDistance(entity, d0)) {
                ...
                Minecraft.getInstance().getItemRenderer().renderStatic(new ItemStack(PHItemRegistry.COLLAR_TAG.get()), ItemDisplayContext.GROUND, lightIn, OverlayTexture.NO_OVERLAY, pose, buffer, entity.level(), entity.getId());
                pose.popPose();
