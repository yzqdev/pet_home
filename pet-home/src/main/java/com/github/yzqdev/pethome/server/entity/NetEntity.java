    ...
    protected void onHit(@Nonnull HitResult result) {
        ...
        if (containsEntity) {
            ...
            if (type == HitResult.Type.ENTITY) {
                ...
                tag.put(PHDataComponents.ENTITY_HOLDER, nbt);
                newStack.setTag(tag);
...
//        }
        if (hasItemStack) {
            nbt.put(entityNbt, itemStack.save(nbt));
    ...
    public void readAdditionalSaveData(CompoundTag nbt) {
        ...
        itemStack = ItemStack.of(nbt.getCompound(PetHomeMod.MODID));
        if (nbt.contains(entityNbt, Tag.TAG_COMPOUND)) {
            ItemStack savedStack = ItemStack.of(nbt.getCompound(entityNbt));
